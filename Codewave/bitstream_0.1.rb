...
live_loop :looper do
  if one_in 5
    ...
    4.times do
      sample :loop_breakbeat, beat_stretch: 2, amp: 0.3
      sleep 2
    end
  end
end
...
live_loop :looper2 do
  if not one_in 5
    sample :loop_mika, beat_stretch: 16, amp: 0.2
  end
  sleep 16
end
...
live_loop :bitstream do
  ...
  if one_in 4
    ...
    with_fx :bitcrusher, mix: 0.4, bits: [5, 6].choose, sample_rate: (range 1000, 3000, step: 250, inclusive: true).choose do
      with_fx :slicer, phase: 0.125, smooth_up: 0.01, smooth_down: 0.01 do
        ...
        with_fx :reverb, mix: 0.3, room: 0.4, amp: 1 do
          notes.size.times do
            ...
            s = play notes.tick, note_slide: 0.5, amp: 0.15, attack: 0.1, sustain: 8, release: 0.1, cutoff: cut.look(:cut), cutoff_slide: 4, res: 0.2
